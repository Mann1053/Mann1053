...
export default function EditRole({ roleId }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Extract permissions IDs from role data
        const permissionIds = roleResponse.permissions.map((p) => p.id);
        ...
        // Fetch all permissions for groups
        const permissionsResponse = await callApi("get", "/permissions");
        setPermissionList(permissionsResponse);

        // Organize permissions into groups
        const groups = permissionsResponse.reduce((acc, permission) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const roleResponse = await callApi("put", `/roles/${roleId}`, {
        name: data.name,
      });
      ...
      const permissionResponse = await callApi("put", `/role-permissions`, {
        roleId: roleResponse.id,
        permissionIds: data.permissions,
      });

      if (permissionResponse?.error) {
        showToast("Role updated but error setting permissions", "error");
        return;
      }
