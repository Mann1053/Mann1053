...
export default function EditPermission({ permissionId }) {
  ...
  const fetchPermission = async () => {
    try {
      const response = await callApi("get", `/permissions/${permissionId}`);
      setValue("name", response.name);
    } catch (err) {
      console.error("Error fetching permission:", err);
      showToast("Error fetching permission", "error");
    } finally {
  ...
  const onSubmit = async (data) => {
    try {
      const result = await callApi("put", `/permissions/${permissionId}`, {
        name: data.name,
      });
      if (result?.error) {
